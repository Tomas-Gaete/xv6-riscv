...
//handles logic for openiong or creating a file
  if(omode & O_CREATE){
    ...
    ilock(ip);
    //Handles case where it's a directory that isn't  in read only mode
  ...
  //Checks if it is a device and its major number is invalid
...
//Handles case where it is a device
  if(ip->type == T_DEVICE){
    f->type = FD_DEVICE;
    f->major = ip->major;
  } else {
